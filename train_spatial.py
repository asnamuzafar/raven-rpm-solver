"""
RAVEN RPM Solver - Experiment 11: Spatial Reasoning

This script trains the Neuro-Symbolic model using the new 'SpatialResNet' encoder.
Expected improvement: The spatial encoder preserves 5x5 grid features and adds CoordConv channels,
enabling the model to solve 'Position' rules which were previously impossible due to average pooling.

Components:
- Encoder: SpatialResNet (CoordConv + No Pooling + Spatial Adapter)
- Reasoner: RuleAwareReasoner (Neuro-Symbolic)
- Loss: CrossEntropy + Contrastive + Attribute Supervision

Usage:
    python train_spatial.py --data_dir ./data/iraven_large --epochs 30 --batch_size 64
"""
import argparse
import random
import numpy as np
...
def contrastive_loss(logits: torch.Tensor, targets: torch.Tensor, margin: float = 1.0) -> torch.Tensor:
    B = logits.shape[0]
...
def compute_attribute_loss(context_features: torch.Tensor, meta: dict, reasoner: RuleAwareReasoner, device: str) -> torch.Tensor:
    ...
    for attr_idx, name in enumerate(attr_names):
        if name in attr_logits and attr_idx < context_gt.shape[1]:
            ...
            gt_clamped = gt.clamp(0, logits.shape[-1] - 1)
            loss = F.cross_entropy(logits.view(-1, logits.shape[-1]), gt_clamped.view(-1))
            total_loss += loss
            num_attrs += 1
...
def validate(model, dataloader, device, epoch, total_epochs, use_amp):
    ...
    with torch.no_grad():
        for batch in pbar:
            ...
            x, y = x.to(device), y.to(device)
            with autocast(enabled=use_amp):
                # We call encoder and reasoner separately because we need to handle potential custom forward
...
def main():
    ...
    print("="*60)
    print("Experiment 11: SPATIAL REASONING + SCL")
    print("="*60)
    print(f"Attr: {args.lambda_attr}, Contrast: {args.lambda_contrast}")
    
    train_dl, val_dl, test_dl = create_dataloaders_with_meta(Path(args.data_dir), args.batch_size, NUM_WORKERS)
    
    # Create model with SPATIAL ENCODER
    model = create_model(
        model_type='neuro_symbolic',
        pretrained_encoder=True,
        freeze_encoder=False,
        use_simple_encoder=False,
        encoder_type='spatial',     # <--- Critical change
        feature_dim=512,
        hidden_dim=256,
        dropout=DROPOUT
    ).to(device)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if val_acc > best_acc:
            best_acc = val_acc
            torch.save(model.state_dict(), save_dir / 'spatial_best.pth')
