"""
RAVEN RPM Solver - Neuro-Symbolic + SCL Training Script

Combines the best performing Neuro-Symbolic model (RuleAwareReasoner) with 
Structure-Consistency Learning (SCL) techniques:
1. Contrastive Loss (Margin Ranking)
2. Attribute Supervision
3. Mixed Precision Training (AMP)

Usage:
    python train_neuro_scl.py --data_dir ./data/iraven_large --epochs 30 --batch_size 64
"""
import argparse
import json
import random
...
def contrastive_loss(
    ...
    """Margin ranking loss: score(correct) > score(incorrect) + margin"""
    B = logits.shape[0]
...
def train_epoch(
    ...
    for batch in pbar:
        ...
        with autocast(enabled=use_amp):
            ...
            # Weighting: Experiment 4 used lambda_attr=0.5. We add lambda_contrast.
...
def validate(model, dataloader, device, epoch, total_epochs, use_amp=True):
    model.eval()
    total_loss = 0.0
    correct = 0
    total = 0
    ...
    with torch.no_grad():
        for batch in pbar:
            ...
            with autocast(enabled=use_amp):
                # We need simple forward here, model() usually calls encoder then reasoner
                logits = model(x)
...
def main():
    ...
    print("="*60)
    print("Neuro-Symbolic + SCL Training")
    print("="*60)
    print(f"Attr Weight: {args.lambda_attr}")
    print(f"Contrast Weight: {args.lambda_contrast}")
    
    train_dl, val_dl, test_dl = create_dataloaders_with_meta(
        Path(args.data_dir), args.batch_size, NUM_WORKERS
    )
    
    # Create standard Neuro-Symbolic model
    model = create_model(
    ...
    for epoch in range(1, args.epochs + 1):
        tm = train_epoch(
            ...
            epoch, args.epochs, args.lambda_attr, args.lambda_contrast, use_amp
        )
        ...
        if val_acc > best_acc:
            best_acc = val_acc
            torch.save(model.state_dict(), save_dir / 'neuro_scl_best.pth')
